...
export default function NoteListItem({noteId, cid, setContent}) {
  const {cid: noteCid, title} = cid
  let firstParagraph = ''
  let enableQuery = false
  if(!noteId) return
  if(noteCid) enableQuery = true;
  ...
  if (isFetching) {
    return (
          <div role="status">
              <svg aria-hidden="true" className="w-8 h-8 mr-2 text-gray-200 animate-spin dark:text-gray-600 fill-emerald-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  if (data && data.value?.content.content.length > 0) {
